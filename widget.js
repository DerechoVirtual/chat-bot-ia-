/* ========================================
   ...
   VERSION: HUGE UI + ERROR HANDLING
   ======================================== */
...
REGLA: SIEMPRE termina con una pregunta que acerque a la demo.`;
    ...
    function injectStyles() {
        const css = `
            ...
            #dv-toggle img { width: 50px; height: 50px; border-radius: 50%; } /* Increased icon */
            #dv-toggle::after {
                content: '1';
                position: absolute;
                top: 0;
            ...
            #dv-header img { width: 50px; height: 50px; border-radius: 50%; background: rgba(255,255,255,0.1); }
            #dv-header-info h4 { margin: 0; font-size: 1.25rem; font-weight: 600; }
            #dv-header-info span { font-size: 1rem; opacity: 0.9; display: flex; align-items: center; gap: 4px; }
    ...
    async function handleSend() {
        ...
        if (text.includes('?') || text.length > 20 && !step.collectField) {
            ...
            try {
                ...
                hideTyping();
                addMessage('Lo siento, hubo un error de conexión. Inténtalo de nuevo.', 'error');
                console.error(err);
            }
            return;
        }
        ...
        if (step && step.collectField) {
            chatState.collectedData[step.collectField] = text;
